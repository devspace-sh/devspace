...
func New(ctx devspacecontext.Context, deployConfig *latest.DeploymentConfig) (deployer.Interface, error) {
	if deployConfig.Tanka == nil {
		return nil, errors.New("error creating tanka deploy config: kubectl is nil")
	} else if deployConfig.Tanka.Path == "" {
		return nil, errors.New("no path defined for tanka deploy")
	}

	// make sure tanka exists
	var (
		err     error
		cmdPath string
	)
	if deployConfig.Tanka.TankaBinaryPath != "" {
		cmdPath = deployConfig.Tanka.TankaBinaryPath
	} else {
