...
// InternalRegistryChartPath is the standard path where the internal registry chart will be written to
const InternalRegistryChartPath = ".devspace/internal-registry"
...
func InternalRegistry() error {
	...
	internalRegistryConfig := &v1.DeploymentConfig{
		Name:      configutil.String(InternalRegistryName),
		Namespace: deployments[0].Namespace,
		Helm: &v1.HelmConfig{
			ChartPath: configutil.String(InternalRegistryChartPath),
		},
	}
	...
	internalRegistryConfig.Helm.Values = &registryReleaseValues
	deployments = append(deployments, internalRegistryConfig)
