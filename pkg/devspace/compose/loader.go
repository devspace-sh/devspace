...
func (cl *configLoader) Load(log log.Logger) (*latest.Config, error) {
	...
	for secretName, secret := range dockerCompose.Secrets {
		...
		if pipelines["dev"] == nil {
			devPipeline, err := pipeline.GetDefaultPipeline("dev")
			if err != nil {
				return nil, err
			}
			pipelines["dev"] = devPipeline.Clone()
		}

		devSecretStep, err := createSecretStep(secretName, cwd, secret)
		if err != nil {
			return nil, err
		}
		pipelines["dev"].Steps = append([]latest.PipelineStep{*devSecretStep}, pipelines["dev"].Steps...)

		if pipelines["purge"] == nil {
			purgePipeline, err := pipeline.GetDefaultPipeline("purge")
			if err != nil {
				return nil, err
			}
			pipelines["purge"] = purgePipeline.Clone()
		}

		purgeSecretStep := deleteSecretStep(secretName)
		pipelines["purge"].Steps = append(pipelines["purge"].Steps, *purgeSecretStep)
	}
...
func createSecretStep(name string, cwd string, secret composetypes.SecretConfig) (*latest.PipelineStep, error) {
	file, err := filepath.Rel(cwd, filepath.Join(cwd, secret.File))
	if err != nil {
		return nil, err
	}

	return &latest.PipelineStep{
		Run: fmt.Sprintf("kubectl create secret generic %s --namespace=${devspace.namespace} --dry-run=client --from-file=%s=%s -o yaml | kubectl apply -f -", name, name, filepath.ToSlash(file)),
	}, nil
}

func deleteSecretStep(name string) *latest.PipelineStep {
	return &latest.PipelineStep{
		Run: fmt.Sprintf("kubectl delete secret %s --namespace=${devspace.namespace} --ignore-not-found", name),
	}
}
