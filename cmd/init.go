...
func (cmd *InitCmd) Run(f factory.Factory, plugins []plugin.Metadata, cobraCmd *cobra.Command, args []string) error {
	...
	configAnnotations := map[string]string{
		...
		"(?m)^(    )(values:)\\s*{}":   "$1$2\n$1# image: image(app):tag(app)\n$1# ingress:\n$1#   enabled: true",
		"(?m)^(image(app):tag(app))":   "$1 # Use the `app` image (see `images`) and the image tag DevSpace generated during image building in your Helm values",
