...
If you want the localhost UI to require token-based auth for sensitive routes and redact sensitive config values in the stack view, start DevSpace with:
```bash
devspace dev --protect-ui
```

You can also use the same flag when starting the UI directly:
```bash
devspace ui --protect-ui
```
