...
1. **Build the Dockerfile(s)** specified in the [`images` section of the `devspace.yaml`](../configuration/images/index.mdx)
2. **Tag** the resulting image(s) with an auto-generated tag according to a [customizable tag schema](../configuration/images/index.mdx)
...
5. **Deploy** everything that is defined within the [`deployments` section in the `devspace.yaml`](../configuration/deployments/index.mdx)
