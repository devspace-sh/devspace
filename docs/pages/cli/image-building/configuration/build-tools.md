...
- [`kaniko`](#TODO) for building images directly inside Kubernetes ([fallback for `docker`](#dockerdisablefallback-kaniko-as-fallback-for-docker))
...
This config option may be useful when developing an application that takes long to build and does not need to be rebuild very frequently because the [file synchronization](#TODO) in development mode is much quicker to update the development container than rebuilding the image. In this case, you could set `disabled: true` and manually rebuild if needed using `devspace build`.
...
- [`target`](/docs/cli/image-building/configuration/build-options#TODO) defining the build target for multi-stage builds
- [`network`](/docs/cli/image-building/configuration/build-options#TODO) to define which network to use during building (e.g. `docker build --network=host`)
- [`buildArgs`](/docs/cli/image-building/configuration/build-options#TODO) to pass arguments to the Dockerfile during the build process
...
- [`target`](/docs/cli/image-building/configuration/build-options#TODO) defining the build target for multi-stage builds
- [`network`](/docs/cli/image-building/configuration/build-options#TODO) to define which network to use during building (similar to `docker build --network=host`)
- [`buildArgs`](/docs/cli/image-building/configuration/build-options#TODO) to pass arguments to the Dockerfile during the build process
...
```
**Explanation:**  
The image `backend` would be built using the command `./build --arg1=arg-value-1 --arg2=arg-value-2 "[IMAGE]:[TAG]"` while `[IMAGE]` would be replaced with the `image` option (in this case: `john/appbackend`) and `[TAG]` would be replaced with the tag generated according to the [tagging schema](#TODO).
...
```
**Explanation:**  
The image `backend` would be built using the command `./build --arg1=arg-value-1 --arg2=arg-value-2 --image="[IMAGE]:[TAG]"` while `[IMAGE]` would be replaced with the `image` option (in this case: `john/appbackend`) and `[TAG]` would be replaced with the tag generated according to the [tagging schema](#TODO).
