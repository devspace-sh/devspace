...
[Learn more about configuring file synchronization using `devspace.yaml`.](../configuration/dev/files/sync.mdx)
...
### Ignore `.git/`
The sync can fail when files are constantly being changed while they are being synchronized. DevSpace will retry failing sync attempts, but for a folder such as `.git/`, which contains continuously changing information and files which may be locked by the IDE, we recommend you [ignore them via the `excludePaths` option](../configuration/dev/files/sync.mdx#exclude-paths) in `devspace.yaml`.
...
- exclude these files using the [`excludePaths` option](../configuration/dev/files/sync.mdx#exclude-paths)
- exclude these files using the [`uploadExcludePaths` option](../configuration/dev/files/sync.mdx#example-exclude-paths-from-synchronization)
- run the containers using a different user during development\*

\* You can achieve this by:
- Adding a `USER` statement in your Dockerfile: This is especially recommended when using multi-stage builds, since you can add the `USER` statement for your development/build stage and add another `USER` statement to your production stage. Then, you can control the build target using the [`options.target` option](../configuration/images/build-settings/target.mdx) for Docker and kaniko builds, and define a profile for removing the build stage for production deployments.
