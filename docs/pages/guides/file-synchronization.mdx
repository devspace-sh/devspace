...
### Ignore `.git/`
The sync can fail when files are constantly being changed while they are being synchronized. DevSpace will retry failing sync attempts but for folder such as `.git/` which contain continuously changing information and files which may be locked by the IDE, we recommmend to [ignore them via the `excludePaths` option](../configuration/dev/files/sync.mdx#exclude-paths) in `devspace.yaml`.
