...
- Importing a library of custom [`functions`](../functions/basics.mdx) that should be available in each project
- Importing a set of standardized [`pipelines`](../pipelines/basics.mdx) to make the `devspace dev` behavior consistent across a set of projects
- Importing a set of custom [`commands`](../commands/basics.mdx) that should be available for engineers in similar project (e.g. you want that all Java Spring Boot applications should provide a `devspace run migrate-db` command)
