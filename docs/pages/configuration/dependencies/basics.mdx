...
It is also possible to reference a dependency's image with the helpers `imageName()`, `imageNameImage()` or `imageNameTag()` in deployment:
```yaml
dependencies:
- name: dep1
  source:
...
deployments:
  - name: quickstart
    helm:
      ...
      values:
        containers:
          - image: imageName(dep1.image1) # -> replaced with 'myusername/devspace:xxxx' 
```
