...
### `noCache`
The `noCache` option expects a boolean that disable the caching behavior for variables. This will prompt the user on every run and will not save the value when set to `true`.
