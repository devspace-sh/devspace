...
- **Installed Commands** (`require.commands`): You can define arbitrary commands such as `kubectl` or `helm` that need to be present locally. You can require a specific version or range of versions, or require that the command exists, can be executed and that outputs a string that matches a regex.
...
```yaml
require:
  # Either 'version', 'executeRegEx' or both, need to be defined per each command
  # If option is defined: by default devspace will try to call the command 'NAME version'
  # and use the regex '(v\\d+\\.\\d+\\.\\d+)' to find the version
  # If executeRegEx is defined: devspace will run the command as 'NAME executeArgs'
  # and use the regex to check its output
  commands:
  - name: helm
    version: '> 3.0.0'
  - name: my-custom-command
    ...
    version: '4.6.7'
  - name: kubectl
    executeRegEx: "^kubectl controls the Kubernetes cluster manager.$"
    executeArgs: ["help"]
```
