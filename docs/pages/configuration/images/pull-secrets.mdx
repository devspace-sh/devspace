...
images:
  ...
  frontend:
    image: gcr.io/john/appfrontend
```
**Explanation:**  
- The first image `backend` will be pushed to Docker Hub and DevSpace will **not** create a pull secret for Docker Hub because `createPullSecret` is `false`. 
  This makes sense for public images where no login is required to pull the image or where you want to manage the pull secret yourself.
- The second image `frontend` will be pushed to gcr.io and DevSpace will create a pull secret for gcr.io because `createPullSecret` defaults to `true`, 
  so Kubernetes can pull the image from gcr.io.
