...
images:                             # map[string]struct | Images to be built and pushed
  image1:                           # string   | Name of the image
    ...
    tagsAppendRandom: true          # bool     | If true appends a random suffix to each defined image tag
...
- The second image `frontend` would be tagged as `appfrontend:[TAG]` and pushed to `gcr.io` using the path `${DEVSPACE_USERNAME}` which is a [dynamic config variable](../../configuration/variables/basics.mdx). 
...
- **DEVSPACE_RANDOM**: A random 6 character long string (prefer `tagsAppendRandom`)
...
images:
  backend:
    ...
    tags: 
    - latest
    - dev-backend-${DEVSPACE_GIT_COMMIT}
```
**Explanation:**  
...
This is an additional option that will append a random suffix to each tag during build. The benefit of choosing `tagsAppendRandom` over `${DEVSPACE_RANDOM}` is that `tagsAppendRandom` reduces the amount of image rebuilds.
Thats because devspace hashes the image config and build context to determine if it should rebuild an image during `devspace dev/build/deploy`. If you use `${DEVSPACE_RANDOM}` in a tag, the variable will be resolved to a random string each time devspace loads the config, which
 means that the image config is each time different and devspace has to rebuild the image on every `devspace dev/build/deploy`. Using `tagsAppendRandom` prevents this issue, since the image config itself stays the same and only a change in the build context or image config itself will trigger a rebuild.
 
### Example: Tagging Images with Random Suffixes
```yaml
images:
  backend-1:
    image: john/appbackend
    # This configuration will tag images in the form of:
    # john/appbackend:dev-backend-0adwe341-asja1
    # Since the config stays the same as long as the git commit
    # stays the same, a rebuild will only occur if a change in the 
    # build context occurred or the git commit hash changed.
    tagsAppendRandom: true
    tags: 
    - dev-backend-${DEVSPACE_GIT_COMMIT}
  backend-2:
    image: john/appbackend
    # This configuration will tag images in the form of:
    # john/appbackend:dev-backend-0adwe341-asja1
    # The image config will look different for every devspace run
    # because of the random string that will be used for DEVSPACE_RANDOM
    # which means that devspace will constantly rebuild the image
    tags: 
    - dev-backend-${DEVSPACE_GIT_COMMIT}-${DEVSPACE_RANDOM}
```
