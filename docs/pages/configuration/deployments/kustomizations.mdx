---
title: Deploy Kustomizations
sidebar_label: kubectl (kustomize)
---

import FragmentKubectlApplyArgs from '../../fragments/kubectl-options-applyArgs.mdx';
import FragmentKubectlBinaryPath from '../../fragments/kubectl-options-binaryPath.mdx';
import FragmentKustomizeBinaryPath from '../../fragments/kustomize-options-binaryPath.mdx';
import FragmentKubectlKustomize from '../../fragments/kubectl-kustomize.mdx';
import FragmentKustomizeKustomizeArgs from '../../fragments/kustomize-options-kustomizeArgs.mdx';
import FragmentUpdateImageTags from '../../fragments/kubectl-updateImageTags.mdx';

To deploy Kustomizations using `kustomize` / `kubectl apply -k`, you need to configure them within the `deployments` section of the `devspace.yaml`.

## Example
```yaml {4}
deployments:
  my-deployment:
    kubectl:
      kustomize: true
      manifests:
      - my-kustomization/
      - another-kustomization/
```

The above example will be executing during the deployment process as follows if the `kustomize` binary is not installed:
```bash
kubectl apply -k my-kustomization/
kubectl apply -k another-kustomization/
```

:::warning Kubectl or Kustomize Required
Kustomization deployments require `kubectl` or `kustomize` to be installed. If both are available, DevSpace will use the `kustomize` binary by default.
:::


## Manifests

### `manifests`
The `manifests` option is mandatory and expects an array of paths that point to directories containing Kustomizations.

#### Example: Manifests
```yaml
deployments:
  backend:
    kubectl:
      kustomize: true
      manifests:
      - my-kustomization/
      - another-kustomization/
```

### `kustomize`

<FragmentKubectlKustomize/>


## General Options

### `name`
The `name` option is optional and expects a string to identify this deployment.

#### Example: Deployment Name
```yaml {2}
deployments:
  backend:
    name: custom-backend-name  # name by default
    kubectl:
      kustomize: true
      manifests:
      - my-kustomization/
      - another-kustomization/
```

### `namespace`
The `namespace` option is optional and expects a string with the namespace used to deploy the manifests.

:::warning
Only use this option if you really need to. Hard-coding the namespace in `devspace.yaml` makes it harder to share the configuration with your colleagues. It is recommended to set the default namespace of the current context instead using:
```bash
devspace use namespace [some-namespace]
```
:::

#### Example: Deployment Namespace
```yaml {3}
deployments:
  backend:
    namespace: some-namespace
    kubectl:
      manifests:
      - backend/
      - backend-extra/
```

### `updateImageTags`

<FragmentUpdateImageTags/>

## Kubectl Options

### `applyArgs`

<FragmentKubectlApplyArgs/>

### `createArgs`
The `createArgs` option expects an array of strings stating additional arguments (and flags) that should be used when calling `kubectl create`.

:::info Kustomize Deployments
DevSpace only uses `kustomize create` to render the manifests using the default flags `--dry-run --output yaml --validate=false`. The actual deployment will be executed using `kubectl apply` after DevSpace has replaced the image tags within the rendered manifests in memory.
:::

#### Default Value for `createArgs`
```yaml
createArgs: []
```

#### Example: Custom Kubectl Args & Flags
```yaml
deployments:
  backend:
    kubectl:
      manifests:
      - backend/
      createArgs:
      - "--recursive"
```
**Explanation:**
Deploying the above example would roughly be equivalent to this command:
```bash
kubectl create --dry-run --output yaml --validate=false --recursive -f backend/
```

### `kubectlBinaryPath`

<FragmentKubectlBinaryPath/>


## Kustomize Options

### `kustomizeArgs`

<FragmentKustomizeKustomizeArgs/>

### `kustomizeBinaryPath`

<FragmentKustomizeBinaryPath/>
