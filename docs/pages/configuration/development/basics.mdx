...
Terminal is configurable by [using the `dev.terminal` configuration section](../../configuration/development/interactive-mode.mdx).
:::
...
### `devspace dev -t`
To start development in terminal mode, run:
```bash
devspace dev -t
```

Learn more about [using terminal](../../guides/interactive-mode.mdx).
