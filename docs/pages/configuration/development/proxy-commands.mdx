---
title: Access Local Commands inside the Container
...
The command that should exist in the container. This command will be created in the container's `/usr/local/bin`.

### `localCommand`

The optional local command the command should refer to. This is essentially the command that is executed locally. By default, this is the same as `command`.
...
dev:
  my-dev:
    imageSelector: ...
    proxyCommands:
    - command: remote-git # Maps the local git command to the remote-git in the container
      localCommand: git
```
